...
const Detail = (props) => {
    ...
    const loadQuotaData = async (lineChart, pieChart) => {
        ...
        if (isAdminUser) {
            url = `/api/log/stat?username=${username}&start_timestamp=${localStartTimestamp}&end_timestamp=${localEndTimestamp}`;
        } else {
            url = `/api/log/self/stat?start_timestamp=${localStartTimestamp}&end_timestamp=${localEndTimestamp}`;
        }
        try {
            ...
            if (success && data) {
                ...
                setQuotaData(chartData);
                updateChart(lineChart, pieChart, chartData);
            } else {
                // 显示默认空数据
                const emptyData = [{
                    'count': 0,
                ...
                setQuotaData(emptyData);
                updateChart(lineChart, pieChart, emptyData);
            }
        } catch (error) {
            console.warn('数据接口不可用:', error.message);
            const emptyData = [{
                'count': 0,
                'model_name': '无数据',
                'quota': 0,

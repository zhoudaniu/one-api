	...
	c.JSON(http.StatusOK, gin.H{
		...
		"data": gin.H{
			...
			"oidc_userinfo_endpoint":      config.OidcUserinfoEndpoint,
				// Midjourney 相关配置
				"enable_drawing":              config.EnableDrawing,
				"mj_notify_enabled":           config.MjNotifyEnabled,
				// 数据看板相关配置
				"enable_data_export":          config.EnableDataExport,
				"data_export_default_time":    config.DataExportDefaultTime,
				// 侧边栏配置
				"default_collapse_sidebar":    config.DefaultCollapseSidebar,
				// 聊天链接2
				"chat_link2":                  config.ChatLink2,
		},
	})
	return
}

...
func GetUserDashboard(c *gin.Context) {
	...
	endOfMonth := time.Date(now.Year(), now.Month()+1, 0, 23, 59, 59, 0, now.Location()).Unix()

	dashboards, err := model.SearchLogsByDayAndModel(id, int(startOfMonth), int(endOfMonth))
	if err != nil {
		dashboards = []*model.LogStatistic{}
	}

	// 计算本月总请求数和 Token 数
	var monthlyRequestCount int
	var monthlyTokenCount int
	for _, dashboard := range dashboards {
		monthlyRequestCount += dashboard.RequestCount
		monthlyTokenCount += dashboard.Quota // 这里 Quota 实际上是 token 使用量
	}

	// 获取配置中的 quota_per_unit

...
func InitOptionMap() {
	...
	// Midjourney 相关配置
	config.OptionMap["EnableDrawing"] = strconv.FormatBool(config.EnableDrawing)
	config.OptionMap["MjNotifyEnabled"] = strconv.FormatBool(config.MjNotifyEnabled)
	// 数据看板相关配置
	config.OptionMap["EnableDataExport"] = strconv.FormatBool(config.EnableDataExport)
	config.OptionMap["DataExportDefaultTime"] = config.DataExportDefaultTime
	// 侧边栏配置
	config.OptionMap["DefaultCollapseSidebar"] = strconv.FormatBool(config.DefaultCollapseSidebar)
	// 聊天链接2
	config.OptionMap["ChatLink2"] = config.ChatLink2
...
func updateOptionMap(key string, value string) (err error) {
	...
	if strings.HasSuffix(key, "Enabled") {
		...
		case "EnableDrawing":
			config.EnableDrawing = boolValue
		case "MjNotifyEnabled":
			config.MjNotifyEnabled = boolValue
		case "EnableDataExport":
			config.EnableDataExport = boolValue
		case "DefaultCollapseSidebar":
			config.DefaultCollapseSidebar = boolValue
		}
	}
	switch key {

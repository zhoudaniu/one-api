...
const TokensTable = () => {
      ...
      render: (text, record, index) => (
        <div>
          <Popover
            ...
            position='top'
          >
            <Button type='tertiary' theme='borderless' style={{ color: 'var(--semi-color-secondary)' }}>查看</Button>
          </Popover>
          <Button
            type='tertiary'
            theme='borderless'
  ...
  const loadTokens = async (startIdx) => {
    ...
    const res = await API.get(
      `/api/token/?p=${startIdx}&size=${pageSize}&order=${orderBy}`
    );
      ...
      <Dropdown
        ...
        render={
          <Dropdown.Menu>
            ...
            <Dropdown.Item
              onClick={() => handleOrderByChange('', { value: 'remain_quota' })}
            >
              按剩余额度排序
            </Dropdown.Item>
            <Dropdown.Item
              onClick={() => handleOrderByChange('', { value: 'used_quota' })}
            >
              按已用额度排序
            </Dropdown.Item>
          </Dropdown.Menu>
